...
public class Validation {
	...
	/** check ssn exist or not
	  * 
	  * @param employees
	  * @param ssn
	  * @return
	  */
	 public boolean checkIdExist(List<Department> departments, String ssn) {
	     for (int i = 0; i < departments.size(); i++) {
	    	  for (int j = 0; j < departments.get(i).getEmployees().size(); j++) {
	    		  if (departments.get(i).getEmployees().get(j).getSsn().equalsIgnoreCase(ssn)) {
