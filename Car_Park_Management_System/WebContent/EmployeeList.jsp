				...
				<div class="container p-5">
							...
							<div class="input-group mb-2 mr-sm-2">
								...
								<select class="select rounded-right border-info">
									<c:forEach items="${employeelist}" var="e">
										<option value="">${e.employeeName}</option>
									</c:forEach>
								</select>
					...
					<div class="row">
						<table class="table table-bordered">
							...
							<tbody>
								<c:forEach items="${listPage}" var="emplist">
									<tr>
										<td>${emplist.employeeId}</td>
										<td>${emplist.employeeName}</td>
										<td>${emplist.employeeBirthdate}</td>
										<td>${emplist.employeeAddress}</td>
										<td>${emplist.employeePhone}</td>
										<td>${emplist.department}</td>
										<td><a href="#" class=""><i class="fa fa-eye"
						...
						<ul class="pagination">
							...
							<c:forEach begin="${1}" end="${requestScope.num}" var="i">
								...
								<li class="page-item "><a class="page-link"
									href="employeeList?data=${data}&&page=${i}">${i}</a></li>
							</c:forEach>
							<li class="page-item"><a class="page-link" href="#"
