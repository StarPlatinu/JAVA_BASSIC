...
public class FruitManager {
	 ...
	 * @param item      id in the list return item
	 */
	...
	public int displayListFruit() {
		...
		for (Fruit fruit : fruits) {
			if (fruit.getQuantity() != 0) {
				...
				System.out.printf("%-10d%-20s%-20s%-15.0f$\n", countItem, fruit.getFruitName(), fruit.getOrigin(),
						fruit.getPrice());
			}
		}
		if (countItem == 0) {
	...
	public void shopping() {
		...
		while (true) {
			...
			if (fruitInOrder != null) {
				...
				if (quantity != 0) {
					listOrder.add(new Fruit(fruit.getFruitId(), fruit.getFruitName(), fruit.getPrice(), quantity,
							fruit.getOrigin()));
				}
